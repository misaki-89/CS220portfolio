...
export default function ClientDemo() {
  ...
  const handleSubmit = useCallback((e) => {
    ...
    if (isInjection) {
      ...
      setLog(
        [
          `[i] Input received: ${username}`,
          `[i] Query executed safely (demo)`,
          `[i] No injection detected`,
        ].join("\n")
      );
    }
  }, [username]);
  ...
  return (
    <div className="mt-12">
      ...
      {/* Form */}
      <h2 className="text-xl font-bold mb-4">SQL Injection Demo Input
</h2>
      <form
      ...
      >
        ...
        <button
          type="submit"
          className="bg-[color:var(--accent)] text-white py-2 rounded hover:opacity-90 transition"
        >
          Run Query
        </button>
      </form>
      ...
      <div className="mb-10">
        ...
        <p className="text-[color:var(--text-secondary)] mb-3">
          The attacker enters a crafted string that contains SQL logic instead of a normal username.
          This input is designed to break out of the intended query structure.
        </p>
        <pre className="bg-black text-red-400 p-3 rounded text-sm">
{steps.step1}
        </pre>
      </div>
      ...
      <div className="mb-10">
        ...
        <p className="text-[color:var(--text-secondary)] mb-3">
          The application builds the SQL query by directly concatenating the user input into the string.
          This lack of validation is the core vulnerability that enables SQL Injection.
        </p>
        <pre className="bg-black text-yellow-300 p-3 rounded text-sm whitespace-pre-wrap">
{steps.step2}
        </pre>
      </div>
      ...
      <div className="mb-10">
        ...
        <p className="text-[color:var(--text-secondary)] mb-3">
          The database evaluates the injected condition. Because <strong>'1'='1'</strong> is always TRUE,
          the entire WHERE clause becomes TRUE, allowing the attacker to bypass authentication.
        </p>
...
{steps.step3}
      ...
      <div className="mb-10">
        ...
        <p className="text-[color:var(--text-secondary)] mb-3">
          Since the WHERE clause is always TRUE, the database returns all user records.
          The application mistakenly treats the attacker as a valid user and grants access.
        </p>
        <pre className="bg-black text-green-400 p-3 rounded text-sm whitespace-pre-wrap">
{steps.step4}
        </pre>
...
{query}
      ...
      {log && (
        <div className="mt-10">
          ...
          <pre className="bg-black text-green-400 p-4 rounded-lg text-sm whitespace-pre-wrap">
{log}
          </pre>
        </div>
      )}
